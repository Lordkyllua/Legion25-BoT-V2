...
module.exports = {
  createClan(name, leaderId){
    const db = load(); const id = idFromName(name);
    if(db.clans[id]) return { ok:false, msg:'Clan already exists' };
    db.clans[id] = { id, name, leader:leaderId, members:[leaderId], bank:0, invites:[] };
    if(!db.users[leaderId]) db.users[leaderId] = { id:leaderId, level:1, xp:0, hp:100, gold:50, inventory:[] };
    db.users[leaderId].clan = id;
    save(db); return { ok:true, clan:db.clans[id] };
  },
  ...
  clanInfo(clanId){ const db = load(); const c = db.clans[clanId]; if(!c) return null; return c; },
