...
module.exports = {
  getProfile(userId){
    const db = load();
    if(!db.users[userId]){ db.users[userId] = { id:userId, level:1, xp:0, hp:100, gold:50, inventory:[] }; save(db); }
    return db.users[userId];
  },
  addXP(userId, amount){
    const db = load(); if(!db.users[userId]) this.getProfile(userId);
    const p = db.users[userId]; p.xp += amount;
