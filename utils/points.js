...
module.exports = {
  addPoints(userId, amount){
    const db = load();
    if(!db[userId]) db[userId] = { points:0, lastDaily:0 };
    db[userId].points += amount;
    save(db);
    return db[userId].points;
  },
  ...
  canClaimDaily(userId, cooldownMs=24*60*60*1000){
    const db = load();
    const now = Date.now();
    const last = db[userId]?.lastDaily || 0;
    return (now - last) >= cooldownMs;
  },
  claimDaily(userId, amount=100){
    const db = load();
    if(!db[userId]) db[userId] = { points:0, lastDaily:0 };
    db[userId].points += amount;
    db[userId].lastDaily = Date.now();
    save(db);
    return db[userId];
  },
  getRanking(limit=10){
    const db = load();
    const arr = Object.entries(db).map(([id, v])=>[id, v.points || 0]);
