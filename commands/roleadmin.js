...
module.exports = {
  name: 'roleadmin',
  description: 'Admin: pick which roles members can self-assign',
  async execute(message) {
    if (!message.member.permissions.has(PermissionsBitField.Flags.ManageRoles)) return message.reply('❌ Missing ManageRoles permission');

    const guildRoles = message.guild.roles.cache.filter(r=>r.name!=='@everyone').map(r=>({ label: r.name, value: r.id }));
    if (!guildRoles.length) return message.reply('No roles found on this server.');
    ...
    const embed = new EmbedBuilder().setTitle('Role Admin').setDescription('Select the roles members can assign').setColor('Orange');
